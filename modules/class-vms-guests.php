...
final class VMS_Guests extends Singleton {
	...
	public static function create_guest( array $data ) {
		...
		// Bust list pages + phone/id lookup caches for this guest.
		...
		do_action( 'vms_guest_created', $guest_id, $validated );
	...
	public static function find_by_phone( string $phone ): ?array 
	{
		...
		$result = VMS_Cache::cached(
			...
			static function () use ( $normalized ) {
				...
				return $row ?: false; // false = cache miss signal, not null
			},
			VMS_Config::CACHE_TTL_MEDIUM
		);

		return $result ?: null; // convert false back to null for callers
	}
	...
	public static function register_visit( int $guest_id, string $visit_date, ?int $host_id = null, ?string $courtesy = null ) {
		...
		$calc = VMS_Visit_Limits::calculate_new_visit_status( $guest_id, $host_id, $visit_date );
		...
		if ( $calc['suspend_guest'] ) {
			self::set_guest_status( $guest_id, VMS_Config::STATUS_SUSPENDED );
		}

		$host = $host_id ? self::get_host_data( $host_id ) : array();

		VMS_Notifications::visit_registered( $guest, $visit, $host );
		...
		if ( $host_id && VMS_Config::VISIT_UNAPPROVED === $calc['visit_status'] && ! $calc['suspend_guest'] ) {
			...
			VMS_Notifications::host_limit_reached( $host_id, $visit_date, $pending_count );
		}

		do_action( 'vms_visit_registered', $visit_id, $visit, $calc );
